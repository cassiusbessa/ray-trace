/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   lights.h                                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: emorshhe <emorshhe@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/09/06 00:40:39 by emorshhe          #+#    #+#             */
/*   Updated: 2025/09/06 00:41:50 by emorshhe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#ifndef LIGHTS_H
# define LIGHTS_H

# include "colors.h"
# include "materials.h"
# include "tuples.h"

typedef struct s_point_light
{
	t_tuple						position;
	t_rgb						intensity;
}								t_point_light;

typedef struct s_point_light_node
{
	struct s_point_light_node	*prev;
	t_point_light				light;
	struct s_point_light_node	*next;
}								t_point_light_node;

typedef struct s_point_light_list
{
	t_point_light_node			*head;
	t_point_light_node			*tail;
	int							count;
}								t_point_light_list;

typedef struct s_lighting_vectors
{
	t_tuple						lightv;
	t_tuple						eyev;
	t_tuple						normalv;
}								t_lighting_vectors;

typedef struct s_lighting_params
{
	t_tuple						position;
	t_tuple						eyev;
	t_tuple						normalv;
	int							in_shadow;
}								t_lighting_params;

// Light operations
t_point_light					new_point_light(t_tuple position,
									t_rgb intensity);
t_rgb							lighting(t_material m, t_point_light light,
									t_lighting_params params);
t_rgb							lighting_no_ambient(t_material m,
									t_point_light light,
									t_lighting_params params);

// Lighting components
t_rgb							diffuse_component(t_material m,
									t_point_light light,
									t_lighting_vectors vectors);
t_rgb							specular_component(t_material m,
									t_point_light light,
									t_lighting_vectors vectors);
t_rgb							calc_diff_spec(t_material m,
									t_point_light light,
									t_lighting_vectors vectors);

#endif
