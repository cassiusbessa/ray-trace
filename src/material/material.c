...
{
    return (
        equal_rgb(m1.color, m2.color) &&
        float_equal(m1.ambient, m2.ambient) &&
        float_equal(m1.diffuse, m2.diffuse) &&
        float_equal(m1.specular, m2.specular) &&
        float_equal(m1.shininess, m2.shininess)
    );
}