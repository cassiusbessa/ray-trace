...
static t_rgb ambient_light_contribution(t_world *world, t_material *mat)
{
    ...
    result = multiply_rgb_by_scalar(
        multiply_rgb_by_rgb(mat->color, world->ambient.color),
        mat->ambient * world->ambient.ratio
    );
    return (result);
}

static t_rgb light_contribution(
    t_world *world, t_material *mat, t_point_light light, t_comps comps)
{
    t_rgb contribution;

    contribution = lighting_no_ambient(
        *mat,
        light,
        comps.point,
        comps.eyev,
...
t_rgb shade_hit(t_world *world, t_comps comps)
{
    ...
    while (current)
    {
        color = add_rgb(color, light_contribution(
            world, mat, current->light, comps));
