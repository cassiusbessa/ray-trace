...
/*   Updated: 2025/08/27 22:31:40 by cassius          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int  test_lighting_surface_in_shadow(void)
{
    int errors = 0;
    ...
    // Passa 1 como in_shadow
    t_rgb result = lighting(m, light, position, eyev, normalv, 1);

    errors += test_check(equal_rgb(result, new_rgb(0.1, 0.1, 0.1)),
        "Lighting with surface in shadow should be (0.1, 0.1, 0.1)");

    if (errors)
        test_failure("Lighting test (surface in shadow) failed");
    else
        test_success("Lighting test (surface in shadow) passed");
...
int run_lighting_tests(void)
{
    ...
    errors += test_lighting_surface_in_shadow();
